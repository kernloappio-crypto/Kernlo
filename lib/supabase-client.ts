...
export function getSupabaseClient(): SupabaseClient | null {
  ...
  try {
    ...
    // Configure Supabase client to work with JWT tokens in localStorage
    // persistSession: true ensures auth context is properly restored for RLS policies
    supabaseInstance = createClient(url, key, {
      auth: {
        storage: typeof window !== 'undefined' ? window.localStorage : undefined,
        autoRefreshToken: false,
        persistSession: true,  // CHANGED: Allow session persistence for RLS
