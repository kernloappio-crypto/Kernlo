...
async function handlePendingConfirmation(
  ...
  if (confirmation.confirmation_step === 'awaiting_confirmation') {
    ...
    if (isConfirmed) {
      ...
      return {
        response: `✅ Logged! ${student}: ${minutes}m ${subject} (${platform}). 🚀`,
        handled: true,
...
export async function POST(req: NextRequest): Promise<NextResponse<SMSResponsePayload>> {
  try {
    ...
    const pending = await handlePendingConfirmation(user.id, messageBody);
    if (pending.handled) {
      return NextResponse.json(
    ...
    if (!nlpResult.success && nlpResult.data) {
      ...
      return NextResponse.json(
        {
          message: `Not sure I understood. Did you mean: ${student} did ${minutes}m of ${subject}?`,
    ...
    if (!nlpResult.success) {
      return NextResponse.json(
        {
          message: 'Could not parse. Try: "Ella did 30m of Math on Khan"',
          status: 'error',
        },
        { status: 200 }
      );
